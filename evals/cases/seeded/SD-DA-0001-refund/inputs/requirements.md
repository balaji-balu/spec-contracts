...
upstream:
  ...
  intent: {id: SPEC-9002, version: 1}
  requirements-analysis: {id: SPEC-9002, version: 1}
  glossary: {version: 2}
...
- acceptance:
  - AC-001.1: Given an Order with no Shipment When the Customer requests Cancellation Then the Order is marked cancelled And the Cancellation is published within 1 minute
...
### REQ-004 · Check shipping before refunding
- type: functional
- context: Payments
- priority: must
- statement: When the Payments context receives an order-cancelled notice, the Payments context shall read the order's shipping status from the Ordering database before issuing a Refund.
- acceptance:
  - AC-004.1: Given an order-cancelled notice When the Ordering database shows the order as shipped Then no Refund is issued
- terms: [Refund]

### REQ-005 · Retry rejected refunds
- type: functional
- context: Payments
- priority: must
- statement: If the payment provider rejects a Refund, then the Payments context shall retry the Refund until it succeeds.
- acceptance:
  - AC-005.1: Given a Refund rejected by the provider When the retry interval of 5 minutes passes Then the Refund is submitted again
- terms: [Refund]

## Non-functional requirements

### REQ-101 · Refund arrival latency
- type: nfr
- nfr_category: performance
- context: Payments
- priority: must
- statement: The Payments context shall confirm that each Refund has reached the Customer's account within 1 minute of receiving the order-cancelled notice.
- acceptance:
  - AC-101.1: Given peak weekly cancellation volume When order-cancelled notices arrive Then p99 time from notice to funds in the Customer's account ≤ 1 min
- terms: [Refund, Customer]
